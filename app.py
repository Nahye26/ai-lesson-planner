...
if topic and goal:
    ...
    st.header("3️⃣ 수업 피드백 입력 (줄바꿈으로 여러 줄 입력 가능)")
    feedback_text = st.text_area("각 단계별 활동명을 포함하여 피드백을 작성해주세요.\n예) '전반부 활동인 흥미 유발 영상 시청이 좋았어요.'")

    if st.button("피드백 분석 및 수업안 수정"):
        if not feedback_text.strip():
            st.warning("피드백을 하나 이상 입력해주세요.")
        else:
            feedbacks = [fb.strip() for fb in feedback_text.split("\n") if fb.strip()]
            ...
            if unmatched:
                ...
                for i, fb in enumerate(unmatched):
                    ...
                    sentiment = simple_sentiment_analysis(fb)
                    if sentiment:
                        phase_feedback[phase_select].append((fb, sentiment))
